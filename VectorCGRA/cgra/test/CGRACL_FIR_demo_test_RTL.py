...
class TestHarness( Component ):
  ...
  def construct( s, DUT, FunctionUnit, FuList, DataType, PredicateType,
    ...
    # Add a TestSinkCL to collect the outputs
    s.sink_out = TestSinkCL( DataType )
    connect( s.dut.tile[3].element.send_out[0].msg, s.sink_out.recv )
...
def test_cgra_universal(paramCGRA = None):
  ...
  rtl_outputs = run_sim( th )
  fl_outputs = run_CGRAFL()

  # Compare outputs
  for rtl_out, fl_out in zip(rtl_outputs, fl_outputs):
    assert rtl_out == fl_out

  print("FIR FL and RTL model outputs match!")
